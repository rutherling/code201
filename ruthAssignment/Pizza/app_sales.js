...
function hourlyTable() {
  ...
  for(ii = 0; ii < finalArray.length; ii++){
    var neighborhoodTR = document.createElement('tr');//each neighborhood gets its own row.
    var neighborhoodTH = document.createElement('th');//create TD
    neighborhoodTH.textContent = finalArray[ii].pizzeria; //object.key
    neighborhoodTR.appendChild(neighborhoodTH);//put data into the row, <TD> to <TR>

    var pizzaHead = document.createElement('th');
    pizzaHead.textContent = "Pizzas Sold"
    neighborhoodTR.appendChild(pizzaHead);
    var deliveryHead = document.createElement('th');
    deliveryHead.textContent = "Number of Deliveries";
    neighborhoodTR.appendChild(deliveryHead);
    var driverHead = document.createElement('th');
    ...
    neighborhoodTable.push(neighborhoodTH);//put the TD into the array.
//not sure I need this?
  //var hourTable = []; //hour table will clear each time the neighborhoods' hours finish iterating.
    for(j = 0; j < finalArray[0].Values.length; j++){
      ...
      pizzaTD.textContent = finalArray[ii].Values[j].countPizza; //put data into the TD
